...
parser.add_option("-M","--masses",default=[],action="append")
...
def drawHists():
  ...
  for truth in truth_mods:
    ...
    for c, stype in enumerate(scanTypes):
      graphDict[truth][stype] = {}
      for ptype in plotTypes:
        graphDict[truth][stype][ptype] = r.TGraphErrors()
        graphDict[truth][stype][ptype].SetLineColor(graphCol[c])
        graphDict[truth][stype][ptype].SetMarkerColor(graphCol[c])
        graphDict[truth][stype][ptype].SetLineWidth(2)
        graphDict[truth][stype][ptype].SetMarkerStyle(r.kFullCircle)
        graphDict[truth][stype][ptype].SetTitle('Category %d - %s'%(options.cat,ptype))
